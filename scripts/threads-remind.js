...
/* 하루에 이만큼 넘게는 안 알립니다. 큐가 꼬여 하루에 몰려 나가는 것을 막는 안전장치입니다.
   PER_DOW 의 최대(평일 3)보다 하나 위여야 합니다. 같은 숫자면 안전장치가 아니라 그냥 상한이 되어서
   그날 마지막 편이 조용히 안 나갑니다. 2026-08-22 에 평일을 3편으로 올리면서 3에서 4로 올렸습니다. */
const DAILY_CAP = 4;
...
/* LEAD_MIN  이만큼 앞이면 이 회차가 그 편을 맡아 예약 시각까지 기다립니다.
   ...
   2026-08-17 에 실제 간격을 세어보고 25분에서 120분으로 올렸습니다.
   워크플로에는 10분마다라고 적혀 있지만 깃허브가 부하에 따라 회차를 미루거나 버립니다.
   하루치를 세니 128번 돌아야 할 것이 17번 돌았고, 평균 간격 80분에 최대 214분이었습니다.
   25분 창은 그 간격이면 그냥 지나갑니다. 그날 21시대 편이 자정을 넘겨 잡히면
   "지난 날짜" 로 보고 큐가 통째로 멈추는데, 워크플로는 성공으로 떠서 조용히 며칠이 갑니다.

   넓혀도 두 번 보내지지는 않습니다. 워크플로의 concurrency(threads-remind) 가
   회차를 겹쳐 돌리지 않고, 맡은 회차는 보낸 뒤에야 ~표시를 남깁니다.
   대신 잡 하나가 최대 두 시간 떠 있으므로 워크플로의 timeout-minutes 도 같이 올려야 합니다. */
const LEAD_MIN = 120;
