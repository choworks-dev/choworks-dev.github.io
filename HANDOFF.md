...
`git-server-guide.html` 이 저장소에 있다. 커밋 `e029c7a` 본문은 "사이트 빌드는 content/ 와
assets/ 만 읽으므로 choworks.dev 에는 배포되지 않는다" 고 적고 있고 **그 말은 맞다.**

    https://choworks.dev/git-server-guide.html                       404  (실측 2026-09-05)
    https://raw.githubusercontent.com/.../git-server-guide.html      200  (로그인 없이, 실측)

**배포되지 않는 것과 공개되지 않는 것은 다르다.** 저장소가 공개라 누구나 읽을 수 있고,
그 안에 집 안 서버의 호스트명·사설 대역 주소·서비스 포트·계정명이 들어 있다.
사설 대역이라 밖에서 바로 닿지는 않는다 — **정찰 정보이지 접근 경로는 아니다.**
지울지 남길지는 대표님 판단이라 손대지 않았다(ID5 경유로 올림).

**이 문단에 그 값들을 옮겨 적지 않았다.** 옮겨 적으면 지적하려던 노출을 이 문서가
한 번 더 하는 셈이다. 실제 값은 그 파일과 ID5 에게 보낸 보고에 있다.
...
1. **`git-server-guide.html` 처리 판단** — 위 "지금 열려 있는 것" 참조. 대표님 판단 대기
2. 이 문서를 세션 끝날 때마다 갱신한다(13조). 없어서 오늘 처음 만들었다
